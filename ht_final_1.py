...
class WorkingSqueezeTracker:
    ...
    def normalize(self, key, value):
        hand = "Left" if key.startswith("Left_") else "Right"
        min_val = self.global_min.get(key, 0.0)
    ...
    def lock_current_as_max(self, hand):
        """Lock the current global max values for one hand only."""
        self.fixed_max[hand] = {
            key: val for key, val in self.global_max.items() if key.startswith(hand + "_")
        }
        print(f"Fixed max values locked for {hand} hand.")

    def clear_fixed_max(self, hand):
        """Unlock fixed max values for one hand."""
        self.fixed_max[hand].clear()
        print(f"ixed max values cleared for {hand} hand.")
